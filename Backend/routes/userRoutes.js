...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const organizations = await Organization.find({ members: user._id });
        
        // If the user is not part of any organization, create one
        if (organizations.length === 0) {
            const organization = new Organization({
                name: `${user.name}'s Organization`, // or any default name you prefer
            ...
            console.log(`Organization created for user: ${user.name}`);
        }
        ...
        res.json({ token, orgId: organization ? organization._id : null });
    } catch (error) {
        console.error(error);
