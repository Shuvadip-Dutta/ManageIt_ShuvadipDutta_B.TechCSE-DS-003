...
router.post('/organizations/:orgId/invite', authenticateToken, async (req, res) => {
    try {
        ...
        const existingInvite = await Invite.findOne({ organization: orgId, invitee: user._id, status: 'pending' });
        ...
        const invite = new Invite({ organization: orgId, invitedBy: req.user.userId, invitee: user._id });
        await invite.save();
